...
/*
long
	ByteBuffer_Shift(ByteBuffer* self, us piece) {
	
	if (!self | !self->buffer | !self->maxSize | !self->space)
		return -1;
	if (piece > self->space)
		return -1;
	
	// data moves to end
	memmove( (self->buffer + self->space - piece), (self->buffer), piece );
	
	self->size += piece;
	self->space -= piece;
	
	// setting space by zeros
	memset( (self->buffer), 0, self->space*sizeof(uc) );
