...
static Data_t* ldata = NULL;
static INTEGER_t temp;
...
mmsMsg_createBasicDataElement(MmsValue* value)
{
    ...
    case MMS_ARRAY:
        {
            ...
            int i;
            for (i = 0; i < size; i++) {
                dataElement->choice.array->list.array[i] = mmsMsg_createBasicDataElement(value->value.array.components[i]);
            }
        }
        break;
    ...
    case MMS_STRUCTURE:
        {
            int size = value->value.structure.componentCount;
...
mmsMsg_parseDataElement(Data_t* dataElement)
{
	...
	if (dataElement->present == Data_PR_structure) {
		...
		if (dataElement->present == Data_PR_integer) {
			...
			//	BerInteger_createFromInt32(dataElement->choice.integer);
			...
			//Asn1PrimitiveValue* berInteger = 
			//	BerInteger_createFromInt32(dataElement->choice.Unsigned);
...
mmsMsg_addResultToResultList(AccessResult_t* accessResult, MmsValue* value)
{
    ...
    break;
		...
		default:
			...
			if (1) printf("MMS read: unknown value type %i in result\r\n", value->type);
			break;
		}
	}
}
