...
Thread
	Thread_create3( char * name, ThreadExecutionFunction func,
                  void* arg, int ssize, int unused, bool autodestroy) {
	...
	if (xTaskCreate(task, (const portCHAR *)name, ssize*configMINIMAL_STACK_SIZE, 
		(void *)thread, uxGetPriority(PRIO_IDLE), &(thread->pthread)) != pdPASS) {
...
Thread
	Thread_create2( ThreadExecutionFunction func, void* arg, int ssize,
                  int unused ) {
...
Thread_create(ThreadExecutionFunction function, void* parameter, bool autodestroy)
{
	...
	if (xTaskCreate(task, (const portCHAR *)"pt0",configMINIMAL_STACK_SIZE, 
		(void *)thread, uxGetPriority(PRIO_IDLE),
...
Thread_destroy(Thread thread){
	...
	// osThreadSuspend Не работает почему-то. Сейчас она выключена дефайном INCLUDE_vTaskSuspend
//	osThreadSuspend(thread->pthread);
  vTaskDelete(thread->pthread); // взамен osThreadTerminate(thread->pthread);
	free(thread);
}
