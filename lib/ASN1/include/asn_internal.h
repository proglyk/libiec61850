...
#define PLATFORM_DEBUG(...)  //printf(__VA_ARGS__)
#define ASN1_DEBUGF(debug, ...) do { if (debug) { \
	PLATFORM_DEBUG(VA_MACRO __VA_ARGS__); }} while (0)
...
//#define LWIP_DEBUGF(debug, ...) do {if (((debug) & LWIP_DBG_ON) && ((debug) & LWIP_DBG_TYPES_ON) && ((s16_t)((debug) & LWIP_DBG_MASK_LEVEL) >= LWIP_DBG_MIN_LEVEL)) {LWIP_PLATFORM_DIAG(VA_MACRO __VA_ARGS__); if ((debug) & LWIP_DBG_HALT) {while(1);}}} while(0)
...
_ASN_STACK_OVERFLOW_CHECK(asn_codec_ctx_t *ctx) {
	if(ctx && ctx->max_stack_size) {
		...
		if(usedstack < -(ptrdiff_t)ctx->max_stack_size) {
			//ASN1_DEBUGF(ASN1_DEBUG, ("Stack limit %ld reached\n", (long)ctx->max_stack_size));
			return -1;
		}
	}
	return 0;
